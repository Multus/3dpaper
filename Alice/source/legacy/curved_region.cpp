#include "../source/geodesic_line.cpp"
...
public:
    ...
    void get_mesh_curved_region() {
        ...
        igl::writeOBJ("test_region" + constant+ ".obj",V,F);
        constant += "1";
    }
private:
    ...
    // std::vector<double> get_rotation(std::vector<double>& coeff, std::vector<Point>& main_tangent) {
    //     // line 
    //     // perp
    //     // perp
    //     // angle
    ...
    void calculate_borders(std::vector<Point>& start_region, std::vector<Point>& flat_geodesic, std::vector<Point>& first_left_border, std::vector<Point>& first_right_border, std::vector<double>& coeff, std::vector<std::pair<double, double>> angles) {
        // std::vector<double>& angles, std::vector<double>& rotation) {
        ...
        for (int i = 1; i < geodesic.size(); ++i) {
            ...
            // std::cout << geodesic[i].get_dist(geodesic[i - 1]) << " | ";
            // std::cout << right_border[right_border.size() - 1].get_dist(right_border[right_border.size() - 2]) << " \n ";
            // std::cout << ((geodesic[i] - geodesic[i - 1]) * angles[i].first).x << " " << ((geodesic[i] - geodesic[i - 1]) * angles[i].first).y << " " << ((geodesic[i] - geodesic[i - 1]) * angles[i].first).z << "\n";
            // std::cout << ((geodesic[i] - geodesic[i - 1])).x << " " << ((geodesic[i] - geodesic[i - 1])).y << " " << ((geodesic[i] - geodesic[i - 1])).z << "\n";
            // std::cout << ((geodesic[i] - geodesic[i - 1]).x << " " << ((geodesic[i] - geodesic[i - 1]).y << " " << ((geodesic[i] - geodesic[i - 1]).z << "\n";

            // Point vector_right = geodesic[i] + vector_right * angles[i] - geodesic[i - 1];
            // Point vector_left = geodesic[i] + vector_left * angles[i] - geodesic[i - 1];

            // const Point& geodesic_last = geodesic[geodesic.size() - 1];
            // Point geo_flat = flat_geodesic[0].get_mid(flat_geodesic[1], coeff[i]);

            // double dist = geo_flat.get_dist(first_right_border[0].get_mid(first_right_border[1], coeff[i]));
            // double t = dist/(sqrt(pow(geodesic_last.x - vector_right.x, 2) + pow(geodesic_last.y - vector_right.y, 2) + pow(geodesic_last.z - vector_right.z, 2)));
            // Point mod_vector = vector_right - geodesic_last;
            // right_border.emplace_back(Point(geodesic[i].x + (mod_vector.x)*t, geodesic[i].y + (mod_vector.y)*t, geodesic[i].z + (mod_vector.z)*t));

            // dist = geo_flat.get_dist(first_left_border[0].get_mid(first_left_border[1], coeff[i]));
            // t = dist/(sqrt(pow(geodesic_last.x - vector_left.x, 2) + pow(geodesic_last.y - vector_left.y, 2) + pow(geodesic_last.z - vector_left.z, 2)));
            // mod_vector = vector_left - geodesic_last;
            // left_border.emplace_back(Point(geodesic[i].x + (mod_vector.x)*t, geodesic[i].y + (mod_vector.y)*t, geodesic[i].z + (mod_vector.z)*t));



            // Point test = Point(geo_flat.x + (vector.x - geo_flat.x)*t, geo_flat.y + (vector.y - geo_flat.y)*t, geo_flat.z + (vector.z - geo_flat.z)*t);
            // std::cout << test.x << " " << test.y << " " << test.z << " | ";
            // std::cout << geodesic[i].x << " " << geodesic[i].y << " " << geodesic[i].z << "\n";
            // std::cout << test.x << " " << test.y << " " << test.z << " | ";
            // std::cout << t << "\n";
        }
    }
...
public:
    Paper(std::string file_name) {
        ...
        while(!file.eof()) {
            ...
            std::cout << start_region[0].x << " " << start_region[0].y << " " << start_region[0].z << " | ";
            std::cout << start_region[1].x << " " << start_region[1].y << " " << start_region[1].z << "\n";
            std::cout << flat_geodesic[0].x << " " << flat_geodesic[0].y << " " << flat_geodesic[0].z << " | ";
            std::cout << flat_geodesic[1].x << " " << flat_geodesic[1].y << " " << flat_geodesic[1].z << "\n";
            std::cout << flat_left_border[0].x << " " << flat_left_border[0].y << " " << flat_left_border[0].z << " | ";
            std::cout << flat_left_border[1].x << " " << flat_left_border[1].y << " " << flat_left_border[1].z << "\n";
            std::cout << flat_right_border[0].x << " " << flat_right_border[0].y << " " << flat_right_border[0].z << " | ";
            std::cout << flat_right_border[1].x << " " << flat_right_border[1].y << " " << flat_right_border[1].z << "\n";
            std::cout << start.x << " " << start.y << " " << start.z << "\n";
            std::cout << main_tangent[0].x << " " << main_tangent[0].y << " " << main_tangent[0].z << " | ";
            std::cout << main_tangent[1].x << " " << main_tangent[1].y << " " << main_tangent[1].z << "\n";
            std::cout << end_side.x << " " << end_side.y << " " << end_side.z << "\n";
            std::cout << dist << "\n\n";
            ...
            // main_tangent = std::vector<Point>{Point(0, -1, 1), Point(0, 0, 0)};
        }
